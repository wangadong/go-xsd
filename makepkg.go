...
var (
	PkgGen = &pkgGen{
		BaseCodePath:             "",
		BasePath:                 "github.com/wangadong/go-xsd-pkg",
...
func newPkgBag(schema *Schema) (bag *PkgBag) {
	...
	bag.imports, bag.impsUsed, bag.lines = map[string]string{}, map[string]bool{}, []string{"//\tAuto-generated by the \"go-xsd\" package located at:", "//\t\tgithub.com/wangadong/go-xsd", "//\tComments on types and fields (if any) are from the XSD file located at:", "//\t\t" + bag.Schema.loadUri, "package go_" + bag.safeName(util.Replace(path.Base(bag.Schema.RootSchema([]string{bag.Schema.loadUri}).loadUri), map[string]string{"xsd": "", "schema": ""})), ""}
	bag.imports[bag.impName] = "github.com/wangadong/go-xsd/types"
...
func (me *PkgBag) checkType(typeSpec string) {
	var dt *declType
	tn := util.Replace(typeSpec, typeRenderRepls)
...
func (me *PkgBag) resolveQnameRef(ref, pref string, noUsageRec *string) string {
	...
	return util.PrefixWithSep(impName, ".", me.safeName(util.PrependIf(ref, pref)))
}
...
func (me *PkgBag) safeName(name string) string {
	return util.SafeIdentifier(name)
}

func (me *PkgBag) xsdStringTypeRef() string {
	return util.PrefixWithSep(me.Schema.XSDNamespacePrefix, ":", "string")
}
...
func (me *declType) equivalentTo(dt *declType) bool {
	...
	return !util.StrEquivalent(sme, sdt)
}
			...
			if len(me.Type) > 0 {
				...
				if PkgGen.AddWalkers && !strings.HasPrefix(myName, idPrefix+"HasAtt") {
					...
					for _, f := range me.Fields {
						...
						} else if strings.HasPrefix(f.finalTypeName, "[]") && bag.walkerTypes[util.Replace(f.finalTypeName, typeRenderRepls)] {
